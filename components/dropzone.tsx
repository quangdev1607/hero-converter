...
export default function Dropzone() {
    ...
    const convert = async (): Promise<any> => {
        ...
        for (let action of tmp_actions) {
            if (action.is_converting && !action.is_converted) {
                const { signal } = action.abortController!;
                try {
                    const { url, output } = await convertFile(ffmpegRef.current, action, signal);
    ...
    const cancelConversion = (file_name: string): void => {
        const action = actions.find((elt) => elt.file_name === file_name);
        if (action && action.abortController) {
            action.abortController.abort(); // Trigger abort
            setActions(
                actions.map((elt) =>
                    elt === action
                        ? {
                              ...elt,
                              is_converting: false,
                              is_error: true, // Mark as error to indicate cancellation
                          }
                        : elt
                )
            );
    ...
    const handleUpload = (data: Array<any>): void => {
        ...
        const imageFiles = data.filter((file: any) => file.type.includes("image"));
        const otherFiles = data.filter((file: any) => !file.type.includes("image"));

        // Push image files first
                ...
                {actions.map((action: Action, i: any) => (
                    ...
                    >
                        ...
                        {action.is_converted ? (
                            ...
                            <span
                                onClick={() => cancelConversion(action.file_name)}
                                className="cursor-pointer hover:underline  flex items-center justify-center text-xl text-foreground"
                            >
                                Cancel
                            </span>
                        ) : (
                    ...
                    {is_done ? (
                        ...
                        >
                            {is_converting ? (
                                ...
                                <span className="text-primary-foreground">Convert Now</span>
                            )}
                        </Button>
                    )}
                </div>
                    ...
                    <div className="space-y-4 text-foreground">
                        {is_hover ? (
                            <>
                                ...
                                <h3 className="text-primary-foreground text-center font-medium text-2xl">Yes, right there</h3>
                            </>
                        ) : (
                            <>
                                <div className="justify-center flex text-6xl">
                                    <FiUploadCloud className="text-primary-foreground" />
                                </div>
                                <h3 className="text-primary-foreground text-center font-medium text-2xl">Click, or drop your files here</h3>
                            </>
                        )}
                    </div>
                </div>
